...
- **LAN exposure is an explicit opt-in**: `HOST=0.0.0.0 ./scripts/dev-fresh.sh`. The launcher prints a prominent warning plus your LAN URL. Binding to all interfaces does not disable Host-header protection: list each LAN hostname you use in `GEV_ALLOWED_HOSTS` (for example, `GEV_ALLOWED_HOSTS=globe.lan`). Understand what opting in means: **every device on that network can drive the proxies and spend your OpenAI / Google / OpenSky / AISStream / TomTom / FIRMS quota** for as long as the server runs. Do this only on networks you trust.
