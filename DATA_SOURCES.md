...
| **Geoverse** | Geolocated AR experience discovery and launch links | Deployment-controlled Geoverse service; content rights remain with the experience publisher | "Geoverse" — registered dynamically when configured |
| **MeshMap API** | Explicitly public geolocated AR pins and content launch links | [MeshMap API documentation](https://docs.meshmap.com/); API access and linked content remain subject to MeshMap/provider terms | "MeshMap" — registered dynamically when configured |
| **ARpoise directory** | Open geolocated AR directory records and ARpoise deep links | [ARpoise](https://arpoise.com/); lightweight maintainer permission/rate-limit confirmation still required before production enablement | "ARpoise · Tamiko Thiel and Peter Graf" — registered dynamically only when enabled |
| **Configured Open AR Cloud OSCP operators** | Operator-specific nearby spatial-content records | [Open Spatial Computing Platform](https://github.com/OpenArCloud/oscp); OSCP is a protocol, while content rights and service terms belong to each configured operator | Configured operator attribution plus "via Open AR Cloud OSCP" |
...
- **AR Experiences.** `/api/ar-content` queries the map viewport center—not browser or device geolocation—with a camera-derived radius capped at 100 km. It normalizes Geoverse, MeshMap, ARpoise, and configured OSCP records into one bounded hotspot contract while retaining the real provider/operator ID for independent filters. Provider descriptors expose `maxRadiusM`, `effectiveRadiusM`, and `coverageLimited`; Geoverse and ARpoise currently cover at most 5 km per request even when the aggregate viewport is wider. Provider secrets remain server-side; successful results are cached for 60 seconds and can be served stale for up to 15 minutes. Expired experiences are hidden unless the user selects **Past**. Launching is always an explicit click and opens the provider link/app; GEV does not fetch, proxy, copy, or redistribute the linked AR assets or Unity bundles. MeshMap returns only pins explicitly marked `public`. ARpoise calls only the raw `/php/dir/web/porpoise.php` directory backend—never its CGI front end, which can inject synthetic nearby defaults—and remains production-disabled until lightweight maintainer permission and rate-limit confirmation. OSCP is not presented as a central repository: every configured operator remains a distinct source and attribution. Configuration is capped at 24 OSCP operators, and an aggregate request fetches no more than four selected providers concurrently.
